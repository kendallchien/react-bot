...
class View(discord.ui.View):
    ...
    def generate_emb_message(self, yes_responders, no_responders):
        ...
        self.win_probability = min(
            max(self.win_probability + random.randint(-10, 10), 0), 100)
    ...
    async def yes_button_callback(self, interaction, button):
        ...
        yes_responders = [response for response in self.responses.keys(
        ) if 'yes' in self.responses[response]]
        self.yes_count = len(yes_responders)
        no_responders = [response for response in self.responses.keys(
        ) if 'no' in self.responses[response]]
